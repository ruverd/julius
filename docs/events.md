...
Usage may name `tokenizerId` only together with a nonempty `tokenizerSource`. These optional fields identify the counter used for comparisons; they do not certify that the provider used that tokenizer. Missing fields remain `null`. Cross-source aliases for one call must agree on both fields; conflicting tokenizer claims require an append-only reconciliation. Task-level output-token comparisons also require all baseline and current calls to share one known provider/model/tokenizer identity, complete coverage, and an explicit comparable-output attestation.
...
Known `costUsd` requires a known model and `costProvenance`. Caller-modeled prices carry price source, date, and model identity. Claude CLI session totals carry client-estimate provenance. An xAI provider-reported charge carries `chargeSource: provider_usage`, `chargeField: usage.cost_in_usd_ticks`, `chargeUnit: usd_ticks_1e10`, and actual model identity. Reports split client estimates, price-modeled amounts, and provider-reported charges; the aggregate estimated-cost field combines only the first two and must not be added to its parts. Raw ticks remain in `rawUsage`; the USD float is a display conversion, not billing-grade decimal arithmetic. Financial savings still need a comparable baseline. Auxiliary and restoration categories are part of the same usage sum; their subtotals are never added again.
