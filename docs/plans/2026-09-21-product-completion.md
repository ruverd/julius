...
- Current increment: xAI response identity can confirm a sent candidate even if usage remains incomplete; request-scope and tool-output reductions now stay separate in reports. A known Julius marker requires explicit prior-transform lineage and recompression permission. Memory supports optional confidence, invalidation conditions, audit timestamps/reasons, and paged metadata-only CLI history. A frozen bilingual benchmark manifest records eligible native/competitor/combination arms, per-candidate denominators, sample-size rationale, and seeded paired order. Archive smoke now runs in the release workflow. These additions do not supply live xAI billing, benchmark outcomes, or signed releases.
- Exploratory validation: six frozen English/Portuguese Claude tasks ran in both baseline and safe-hook arms. Both passed five strict tasks; the safe arm cost more in client estimates. A later runner revision records a pre-execution manifest and partial attempts, while an offline analyzer checks hashes, order, grading, and cache-normalized totals. See [pilot evidence](../live-claude-pilot.md). This is not a representative non-inferiority study or proof of token savings.
