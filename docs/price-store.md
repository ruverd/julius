...
`julius savings --task <id> --explain` loads a stored USD snapshot only when a usage event or supplied baseline call explicitly names its `priceSnapshotId` and its provider/model identity matches the snapshot. This is historical price resolution, not automatic price selection. Missing IDs, non-USD rates, missing rate categories, and unknown models leave modeled money unavailable. A snapshot ID reused for conflicting model identities is rejected. `--prices <file>` remains an explicit alternative and bypasses the local store for that report.
